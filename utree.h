...
private:
    ...
    UNode* insert(Account newAcct, UNode*& node);
    UNode* retrieve(string username, UNode* node);
    int max(int a, int b);
    int getHeight(UNode* node);
    UNode* rotateRight(UNode* y);
    UNode* rotateLeft(UNode* x);
    UNode* rotateLR(UNode* node);
    UNode* rotateRL(UNode* node);
    UNode* removeUser(string username, UNode * nodet);
