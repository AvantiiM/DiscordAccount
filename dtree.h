...
private:
    ...
    enum nodeAttrib {size, vacant, users, min_disc, max_disc};

    /* IMPLEMENT (optional): any additional helper functions here */
    /* Recursive implementations */
    bool insert(Account newAcct, DNode*& node);
    bool remove(int disc, DNode*& removed, DNode* node);
    DNode* retrieve(int disc, DNode* node);
    void clear(DNode* node);
    void printAccounts(DNode* node) const;
    void treeToArray(DNode* node, DNode** arr, int& i);
    void arrayToTree(DNode*& node, DNode** arr, int upper, int lower = 0);
    void copyTree(DNode*& dest, DNode* src);
    int getNodeAttribute(DNode* node, nodeAttrib attrib);
};
